...
export default function RiskIndicator({ value, label, showPercent = true, className }: RiskIndicatorProps) {
  ...
  return (
    <div className={cn('bg-secondary-50 rounded-lg p-4', className)}>
      <div className="text-xs uppercase text-secondary-500 font-semibold mb-2">{label}</div>
      <div className="flex items-end">
        <div className="h-2 w-full bg-secondary-200 rounded-full overflow-hidden">
          <div className={cn('h-full rounded-full', bgColor)} style={{ width: `${normalizedValue}%` }}></div>
        </div>
        {showPercent && <div className={cn('ml-2 text-xs font-semibold', textColor)}>{normalizedValue}%</div>}
      </div>
