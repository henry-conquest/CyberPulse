...
const PhishingResistantMfaWidget = ({ tenantId }: PhishingResistantMfaWidgetProps) => {
    ...
    return (
      <Card className="h-full">
        <CardHeader className="pb-2">
          ...
          <CardDescription>Authentication methods that resist phishing attacks</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center h-24 text-center">
            <AlertTriangle className="h-10 w-10 text-amber-500 mb-2" />
            <p className="text-sm text-muted-foreground">Could not load MFA policy data</p>
    ...
    recommendations = [
      {
        ...
        description:
          'SMS authentication is not phishing-resistant as it relies on one-time passcodes sent via text message which can be intercepted.',
        remediation:
          'Disable the SMS authentication method in Microsoft Entra ID to enforce phishing-resistant authentication.',
        impact: 'High',
        category: 'Identity',
        securityLevel: 'Insecure',
      ...
      },
      {
        ...
        remediation:
          'Configure additional security controls for Microsoft Authenticator or replace with fully phishing-resistant methods.',
        impact: 'Medium',
        category: 'Identity',
        securityLevel: 'Partially Secure',
        service: 'Microsoft Entra ID',
  ...
  const securityPercentage =
    totalEnabledMethods === 0 ? 0 : Math.round((enabledPhishResistantMethods.length / totalEnabledMethods) * 100);
      ...
      <CardContent>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            >
              <div className="text-center">
                ...
                <div
                  ...
                  ${riskLevel === 'LOW' ? 'text-green-700' : riskLevel === 'MEDIUM' ? 'text-amber-700' : 'text-red-700'}
                `}
                >
          ...
          <DialogTrigger asChild>
            <Button variant="outline" size="sm" className="w-full mt-4" onClick={handleOpenDialog}>
              View Details
            </Button>
          </DialogTrigger>
                    ...
                    {Object.keys(methodDetails)
                      ...
                      .map((methodId) => {
                        ...
                        return (
                          <div
                            key={methodId}
                            className={`mb-2 p-2 rounded-md ${isEnabled ? 'bg-green-50 border border-green-100' : 'bg-gray-50 border border-gray-100'}`}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                ...
                                <p className={`text-xs mt-1 ${isEnabled ? 'text-green-700' : 'text-gray-400'}`}>
                                  {method.description}
                                </p>
                              </div>
                              <Badge
                                className={isEnabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-400'}
                              >
                    ...
                    {Object.keys(methodDetails)
                      ...
                      .map((methodId) => {
                        ...
                        return (
                          <div
                            key={methodId}
                            className={`mb-2 p-2 rounded-md ${isEnabled ? 'bg-amber-50 border border-amber-100' : 'bg-gray-50 border border-gray-100'}`}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <span className={`font-medium ${isEnabled ? 'text-amber-800' : 'text-gray-500'}`}>
                                  {method.methodName}
                                </span>
                                <p className={`text-xs mt-1 ${isEnabled ? 'text-amber-700' : 'text-gray-400'}`}>
                                  {method.description}
                                </p>
                              </div>
                              <Badge
                                className={isEnabled ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-400'}
                              >
                                {isEnabled ? 'Enabled' : 'Disabled'}
                              </Badge>
                            </div>
                    ...
                    {Object.keys(methodDetails)
                      ...
                      .map((methodId) => {
                        ...
                        return (
                          <div
                            key={methodId}
                            className={`p-2 rounded-md ${isEnabled ? 'bg-red-50 border border-red-100' : 'bg-gray-50 border border-gray-100'}`}
                          >
                            <div className="flex justify-between items-start">
                              <div className="flex-1">
                                <span className={`font-medium ${isEnabled ? 'text-red-800' : 'text-gray-500'}`}>
                                  {method.methodName}
                                </span>
                                <p className={`text-xs mt-1 ${isEnabled ? 'text-red-700' : 'text-gray-400'}`}>
                                  {method.description}
                                </p>
                              </div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg mt-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <Badge
                        className={
                          enabledPhishResistantMethods.length > 0 ? 'bg-green-100 text-green-800' : 'bg-gray-100'
                        }
                      >
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <Badge
                        className={
                          enabledInsecureMethods.length > 0 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                        }
                      >
                          ...
                          {recommendations
                            ...
                            .map((recommendation) => (
                              <li
                                ...
                                className="text-sm bg-red-50 p-3 rounded-md border border-red-200"
                              >
                        ...
                        <ul className="space-y-3">
                          {recommendations
                            ...
                            .map((recommendation) => (
                              <li
                                key={recommendation.id}
                                className="text-sm bg-amber-50 p-3 rounded-md border border-amber-200"
                              >
                                <div className="font-medium mb-1">{recommendation.title}</div>
                                <p className="text-muted-foreground mb-2">{recommendation.description}</p>
                        ...
                        <ul className="space-y-3">
                          {recommendations
                            ...
                            .map((recommendation) => (
                              <li
                                key={recommendation.id}
                                className="text-sm bg-green-50 p-3 rounded-md border border-green-200"
                              >
                                <div className="font-medium mb-1">{recommendation.title}</div>
                                <p className="text-muted-foreground mb-2">{recommendation.description}</p>
