...
export default function RiskGauge({ value, size = 'md', label, className, showLabel = true }: RiskGaugeProps) {
  ...
  return (
    <div className={cn('relative flex flex-col items-center justify-center', className)}>
      {label && <div className="text-sm uppercase text-secondary-500 font-semibold mb-2">{label}</div>}
      ...
      <div className={cn('relative', dimensions[size])}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <div className={cn('font-bold text-secondary-900', textSizes[size])}>{normalizedValue}%</div>
            {showLabel && (
              <div
