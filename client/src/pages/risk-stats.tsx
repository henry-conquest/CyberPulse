    ...
    return (
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Secure Score recommendations are provided directly from Microsoft and cannot be manually managed. The system
            will automatically display all available Microsoft recommendations.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-sm text-blue-700 mt-1">
                  These recommendations come directly from Microsoft's Security portal "To address" list and are
                  automatically updated when your Microsoft Secure Score changes.
                </p>
              </div>
            </div>
          </div>
  ...
  const saveRecommendations = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: [`/api/tenants/${tenantId}/widget-recommendations`],
      });
      queryClient.invalidateQueries({
        queryKey: [`/api/tenants/${tenantId}/widget-recommendations/${widgetType}`],
      });
  ...
  return (
    <DialogContent className="max-w-4xl">
      <DialogHeader>
        ...
        <DialogDescription>
          Select recommendations to show for this {widgetType === 'SECURE_SCORE' ? 'Secure Score' : 'Device Score'}{' '}
          widget
        </DialogDescription>
      </DialogHeader>
            ...
            {filteredRecommendations.map((rec) => (
              <div
                ...
                className={cn(
                  ...
                  selectedRecommendations.includes(rec.id) ? 'border-primary bg-primary/5' : 'hover:bg-muted'
                )}
                ...
                <div className="flex items-start">
                  <div className="mr-4 mt-0.5">
                    <div className="h-5 w-5 border rounded flex items-center justify-center">
                      {selectedRecommendations.includes(rec.id) && <Check className="h-3.5 w-3.5 text-primary" />}
                    </div>
                  </div>
      ...
      <DialogFooter>
        <Button variant="outline" onClick={onClose} disabled={saveRecommendations.isPending}>
          Cancel
        </Button>
        <Button onClick={() => saveRecommendations.mutate()} disabled={saveRecommendations.isPending}>
    ...
    <DialogContent className="max-w-3xl">
      ...
      <div className="py-4">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          {priorityFilter && (
            <Button variant="outline" size="sm" onClick={() => setPriorityFilter(null)} className="ml-auto">
              Show All
            </Button>
          )}
        </div>
...
const SecureScoreRecommendationsDialog = ({
  ...
  useEffect(() => {
    ...
    queryClient.removeQueries({
      queryKey: [`/api/tenants/${tenantId}/secure-score-recommendations`],
    });
        ...
        <div className="flex items-center mb-6">
          ...
          <div>
            <h3 className="text-lg font-medium">
              Current Secure Score: {secureScore.toFixed(1)}/{maxScore}
            </h3>
            <p className="text-muted-foreground">
  ...
  return (
    <Card className={cn('border', getBgColor(score))}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className={cn('p-1 rounded', getIconBgColor(score))}>{icon}</div>
      ...
      <CardContent>
        <div className="mb-3">
          ...
          <Progress
            value={score}
            className={cn('h-2', score < 50 ? 'bg-green-100' : score < 75 ? 'bg-amber-100' : 'bg-red-100')}
            style={
              {
                '--progress-color': score < 50 ? '#22c55e' : score < 75 ? '#eab308' : '#ef4444',
  ...
  return (
    <div className="container mx-auto p-6">
            ...
            <div className="flex items-center">
              <div className="w-32 h-32 mr-6">
                <CircularProgressbar
                  ...
                  styles={buildStyles({
                    pathColor: overallRiskScore < 50 ? '#22c55e' : overallRiskScore < 75 ? '#eab308' : '#ef4444',
                    textColor: overallRiskScore < 50 ? '#22c55e' : overallRiskScore < 75 ? '#eab308' : '#ef4444',
                    trailColor: '#e5e7eb',
                    textSize: '22px',
                  })}
                />
              ...
              <div>
                <div className="flex items-center mb-2">
                  <span className={cn('text-lg font-medium', overallTextColor)}>{overallRiskLevel} Risk</span>
                </div>
                <p className="text-gray-600">
                  This organization has a {overallRiskLevel.toLowerCase()} overall cyber risk score based on analysis of
                  identity security, security awareness training, device security, cloud security, and threat
                  protection.
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="md:col-span-1">
          <GlobalAdminsWidget tenantId={Number(tenantId)} />
        </div>

        {/* MFA Users Widget */}
        <div className="md:col-span-1">
          <MfaUsersWidget tenantId={Number(tenantId)} />
        </div>

        {/* Phishing-Resistant MFA Widget */}
        <div className="md:col-span-1">
          <PhishingResistantMfaWidget tenantId={Number(tenantId)} />
        </div>
      </div>
