...
export default function IntegrationsPage({ tab, action }: { tab?: string; action?: string }) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const needsReconnection = microsoft365OAuthConnections.some(
        (conn: { needsReconnection?: boolean }) => conn.needsReconnection === true
      );
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        if (errorMessage.includes('client_id') || errorMessage.includes('Client ID')) {
          ...
          errorDetails =
            "The redirect URI doesn't match the one configured in your Azure app registration. It must match exactly.";
    ...
    <div className="container mx-auto py-6">
        ...
        <TabsContent value="microsoft365" className="mt-6">
          <Card>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {microsoft365OAuthConnections.map((connection: any) => (
                          <div
                            key={connection.id}
                            className={`flex items-center justify-between p-4 border rounded-lg ${connection.needsReconnection ? 'bg-red-50 border-red-200' : 'bg-slate-50'}`}
                          >
                            <div>
                              <h3 className="font-medium">{connection.tenantName}</h3>
                              <p className="text-sm text-muted-foreground">{connection.tenantDomain}</p>
                              ...
                              {connection.needsReconnection ? (
                                <div className="flex flex-col items-end">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      setLocation(
                                        `/settings?tab=microsoft365&action=connect&company=${connection.companyId || ''}`
                                      );
                                    }}
            ...
            <CardFooter className="flex justify-end">
              <Button onClick={openConnectDialog} disabled={isConnecting}>
                {isConnecting ? (
                  <>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-muted-foreground mb-4">Coming soon - NinjaOne integration is under development.</p>
              </div>
            </CardContent>
            <CardFooter className="flex justify-end">
              <Button disabled>Connect to NinjaOne</Button>
            </CardFooter>
          </Card>
        </TabsContent>
      </Tabs>
      ...
      <Dialog open={connectDialogOpen} onOpenChange={setConnectDialogOpen}>
        <DialogContent className="sm:max-w-[550px]">
          <DialogHeader>
            <DialogTitle className="flex items-center">Connect Microsoft 365 Tenant</DialogTitle>
            <DialogDescription>
              Each Microsoft 365 tenant requires its own set of credentials. These are used to connect to the specific
              tenant and retrieve security information. Please enter your Microsoft Graph API credentials for this
              tenant and select which company it should be associated with.
            </DialogDescription>
          </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="Enter Microsoft Graph Client ID" {...field} />
                    </FormControl>
                    <FormDescription>
                      The client ID from your Azure app registration. This is available in the Azure portal under "App
                      registrations".
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input type="password" placeholder="Enter Microsoft Graph Client Secret" {...field} />
                    </FormControl>
                    <FormDescription>
                      The client secret from your Azure app registration. You can create a new client secret in the
                      Azure portal under "App registrations" → "Certificates & secrets".
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="https://your-domain.com/api/auth/microsoft365/callback" {...field} />
                    </FormControl>
                    <FormDescription>
                      The redirect URI you configured in Azure. This default value should work in most cases. You must
                      register this exact URL in your Azure app registration under "Authentication" → "Redirect URIs".
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        {isLoadingCompanies ? (
                          ...
                          <div className="text-center p-2 text-muted-foreground">No companies available</div>
                        )}
                      </SelectContent>
                    </Select>
                    <FormDescription>
                      Select the company in your system that this Microsoft 365 tenant belongs to. This mapping allows
                      you to organize tenant data by company.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <DialogFooter className="pt-4">
                ...
                <Button type="submit" disabled={form.formState.isSubmitting || isConnecting}>
      ...
      <Dialog open={connSuccessDialog} onOpenChange={setConnSuccessDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Your Microsoft 365 tenant has been connected successfully.</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <p>
              You can now access security insights and metrics for your Microsoft 365 tenant. View security data on the
          ...
          <DialogFooter>
            ...
            <a href="/security-insights">
              <Button>View Security Insights</Button>
            </a>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={connErrorDialog} onOpenChange={setConnErrorDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>There was an error connecting to Microsoft 365.</DialogDescription>
          </DialogHeader>
          <div className="py-4">
            ...
            <div className="space-y-3">
              ...
              {errorMessage.toLowerCase().includes('client_id') || errorMessage.toLowerCase().includes('client id') ? (
              ...
              errorMessage.toLowerCase().includes('client secret') ? (
                <div className="bg-amber-50 border border-amber-200 rounded-md p-3">
                  ...
                  <p className="text-sm mt-1 text-amber-700">
                    ...
                    secret value from your Azure app registration. Remember that client secrets are time-limited and may
                    have expired.
                  </p>
                </div>
              ) : null}
              ...
              {errorMessage.toLowerCase().includes('permission') || errorMessage.toLowerCase().includes('consent') ? (
                <div className="bg-amber-50 border border-amber-200 rounded-md p-3">
                  <h5 className="text-sm font-medium text-amber-800 flex items-center">
                    <AlertTriangle className="h-4 w-4 mr-2" /> Permission/Consent Issue
                  </h5>
                  <p className="text-sm mt-1 text-amber-700">
                    Your application may be missing required permissions or admin consent. Ensure your Azure app has the
                    following Microsoft Graph API permissions:
                    <span className="block mt-1 bg-white p-2 rounded border text-xs font-mono">
                      SecurityEvents.Read.All, Reports.Read.All, Directory.Read.All,
                      <br />
              ...
              <ul className="text-sm space-y-2 ml-5 list-disc">
                <li>
                  <span className="font-medium">Client ID or Secret issues:</span> Double-check that you've entered the
                  correct Azure app registration credentials.
                </li>
                <li>
                  <span className="font-medium">Redirect URI mismatch:</span> Ensure the redirect URI in Azure matches
                  exactly what you've entered.
                </li>
                <li>
                  <span className="font-medium">Missing permissions:</span> Verify your Azure app has the proper
                  Microsoft Graph API permissions.
                </li>
                <li>
                  <span className="font-medium">Admin consent required:</span> Your tenant admin may need to approve the
                  permissions.
                </li>
              </ul>
