        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <CardContent>
              {!latestReport ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Generate a report to see secure score data</p>
                </div>
              ) : (
                <div className="flex items-center">
                  <div className="w-24 h-24 mr-4">
                    <CircularProgressbar
                      value={latestReport.secureScorePercent || 0}
                      text={`${latestReport.secureScorePercent || 0}%`}
                      styles={buildStyles({
                        pathColor: latestReport.secureScorePercent >= 70 ? '#22c55e' : latestReport.secureScorePercent >= 40 ? '#eab308' : '#ef4444',
                        textColor: latestReport.secureScorePercent >= 70 ? '#22c55e' : latestReport.secureScorePercent >= 40 ? '#eab308' : '#ef4444',
                        trailColor: "#e5e7eb",
                        textSize: "22px",
                      })}
                    />
                  ...
                  <div>
                    <div className="flex items-center">
                      {latestReport.secureScorePercent >= 70 ? (
                        <Check className="h-6 w-6 text-green-500" />
                      ) : latestReport.secureScorePercent >= 40 ? (
                    ...
                    <p className="text-sm text-gray-600 mt-1">
                      Score: {latestReport.secureScore?.toFixed(1) || 0} / 278
                    </p>
                    <p className="text-sm text-gray-600">
                      {latestReport.secureScorePercent < 40 && "Urgent action required"}
                      {latestReport.secureScorePercent >= 40 && latestReport.secureScorePercent < 70 && "Improvement needed"}
                      {latestReport.secureScorePercent >= 70 && "Good security posture"}
                    </p>
                  </div>
                </div>
              )}
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Based on your latest report</CardDescription>
            </CardHeader>
            <CardContent>
              {!latestReport ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">Generate a report to see security metrics</p>
                </div>
              ) : (
                <ul className="space-y-2">
                  <li className="flex items-start">
                    <div className="h-5 w-5 rounded-full flex items-center justify-center mr-2 bg-blue-100 text-blue-600">
                      ℹ️
                    </div>
                    <span>
                      Secure Score: {latestReport.secureScorePercent}% ({latestReport.secureScore} points)
                    </span>
                  </li>
                  <li className="flex items-start">
                    <div className="h-5 w-5 rounded-full flex items-center justify-center mr-2 bg-green-100 text-green-600">
                      ✓
                    </div>
                    <span>
                      Report Generated: {new Date(latestReport.createdAt).toLocaleDateString()}
                    </span>
                  </li>
                  <li className="flex items-start">
                    <div className="h-5 w-5 rounded-full flex items-center justify-center mr-2 bg-amber-100 text-amber-600">
                      !
                    </div>
                    <span>
                      Report Status: {latestReport.status || "New"}
                    </span>
                  </li>
                </ul>
              )}
