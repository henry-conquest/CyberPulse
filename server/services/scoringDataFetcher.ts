...
> = {
  microsoft365Admins: async ({ accessToken }) => {
    ...
    for (const role of adminRoles) {
      const membersRes = await fetch(`https://graph.microsoft.com/v1.0/directoryRoles/${role.id}/members`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  riskySignInPolicies: async ({ accessToken }) => {
    ...
    const matchingPolicy = policies.find((policy: any) => {
      ...
      const hasRiskLevel = signInRiskLevels.some((risk: string) => allowedRiskLevels.includes(risk.toLowerCase()));
