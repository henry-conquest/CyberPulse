...
app.get('/api/encrypted-devices/:userId', isAuthenticated, async (req, res) => {
  ...
  app.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const connection = await storage.updateMicrosoft365Connection(existingConnection.id, validatedData);
  ...
  app.get(
    '/api/tenants/:id/microsoft365/global-administrators',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      try {
        const userId = (req.user as any).id;
        ...
        if (!connection) {
          return res.status(404).json({
            message: 'No Microsoft 365 connection found for this tenant',
          });
        }
        ...
        if (process.env.NODE_ENV === 'development' && (!admins || admins.length === 0)) {
          console.log(
            'Warning: No administrators found from Microsoft Graph API. Using sample data for UI development purposes.'
          );
  ...
  app.get(
    '/api/tenants/:id/microsoft365/users-without-mfa',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      try {
        const userId = (req.user as any).id;
  ...
  app.get(
    '/api/tenants/:id/microsoft365/auth-methods-policy',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      try {
        const userId = (req.user as any).id;
        ...
        if (!policy) {
          return res.status(404).json({
            message: 'No authentication methods policy found for this tenant',
          });
        }
        ...
        policy.enabledInsecureMethods.forEach((methodId: string) => {
          ...
          switch (methodId) {
            case 'email':
              ...
              description =
                'Email authentication is not phishing-resistant as it relies on one-time passcodes sent to email which can be intercepted.';
              break;
            case 'sms':
              methodName = 'SMS Authentication';
              description =
                'SMS authentication is not phishing-resistant as it relies on one-time passcodes sent via text message which can be intercepted via SIM swapping, social engineering, or interception.';
              break;
            case 'softwareOath':
              methodName = 'Authenticator App (TOTP)';
              description =
                'Time-based one-time password (TOTP) authenticator apps are susceptible to phishing as codes can be stolen via fake websites.';
              break;
            case 'phoneAuthentication':
              methodName = 'Phone Authentication';
              description =
                'Phone authentication is not phishing-resistant as it relies on voice calls which can be redirected or socially engineered.';
              break;
            case 'password':
              methodName = 'Password';
              description =
                'Passwords are not phishing-resistant as they can be easily captured through phishing websites.';
              break;
            default:
              methodName = methodId;
        ...
        policy.enabledPartiallySecureMethods.forEach((methodId: string) => {
          ...
          switch (methodId) {
            case 'microsoftAuthenticator':
              ...
              description =
                'Microsoft Authenticator in Push mode is partially resistant, but vulnerable to MFA fatigue and push bombing attacks unless number matching and app context are enforced.';
              break;
            case 'x509CertificateSingleFactor':
              methodName = 'X.509 Certificate Authentication';
              description =
                'Certificate-based authentication can be strong, but the current configuration shows x509CertificateSingleFactor, which is not true MFA.';
              break;
            default:
              methodName = methodId;
        ...
        policy.enabledPhishResistantMethods.forEach((methodId: string) => {
          ...
          switch (methodId) {
            case 'fido2':
              ...
              description =
                'FIDO2 security keys offer strong phishing resistance due to cryptographic challenge-response.';
              break;
            ...
            case 'certificateBasedAuthentication':
              ...
              description =
                'Certificate-based authentication offers strong phishing resistance when configured properly.';
              break;
            case 'temporaryAccessPass':
              methodName = 'Temporary Access Pass';
              description =
                'Temporary Access Pass can be phishing resistant if used only for initial setup (not for daily sign-in) and reuse is restricted.';
              break;
            default:
              methodName = methodId;
  ...
  app.get(
    '/api/tenants/:id/microsoft365/security-insights',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const tenantId = parseInt(req.params.id);
        ...
        const securityInsights = {
          ...
          deviceCompliance: {
            ...
            compliancePercentage:
              deviceCompliance.totalDevices > 0
  ...
  app.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const connection = await storage.updateNinjaOneConnection(existingConnection.id, validatedData);
  ...
  app.post(
    '/api/tenants/:id/reports',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ANALYST]),
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const tenantId = parseInt(req.params.id);
  ...
  app.get(
    '/api/tenants/:tenantId/reports/:id',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!isAuthorized) {
        return res.status(403).json({
          message: 'Only administrators and analysts can edit report content',
        });
      }
  ...
  app.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (status) {
        ...
        if (
          status === ReportStatus.ANALYST_READY &&
          ![UserRoles.ADMIN, UserRoles.ANALYST].includes((await storage.getUser(userId))?.role as any)
        ) {
          return res.status(403).json({
            message: 'Only analysts can mark reports as analyst ready',
          });
        }

        // Only admin can mark as manager ready
        if (status === ReportStatus.MANAGER_READY && !((await storage.getUser(userId))?.role === UserRoles.ADMIN)) {
          return res.status(403).json({
            message: 'Only administrators can mark reports as manager ready',
          });
        }
  ...
  app.patch(
    '/api/reports/:id/analyst-notes',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ANALYST_NOTES]),
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const reportId = parseInt(req.params.id);
  ...
  // Report recipients
  app.post(
    '/api/reports/:id/recipients',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ACCOUNT_MANAGER]),
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const reportId = parseInt(req.params.id);
  ...
  app.delete(
    '/api/reports/:id/recipients/:recipientId',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ACCOUNT_MANAGER]),
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
  ...
  app.post(
    '/api/tenants/:tenantId/reports/:reportId/refresh',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ANALYST]),
    asyncHandler(async (req, res) => {
      const tenantId = parseInt(req.params.tenantId);
  ...
  app.post(
    '/api/tenants/:id/generate-quarterly-report',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ANALYST]),
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      ...
      try {
        ...
        if (!report) {
          return res.status(400).json({
            message:
              'Failed to generate quarterly report. It may already exist for this quarter or there may be an issue with the tenant connections.',
          });
        }
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (report.status !== ReportStatus.MANAGER_READY) {
        return res.status(400).json({
          message: 'Report must be marked as manager ready before sending',
        });
      }
  ...
  // Recommendations
  app.get(
    '/api/tenants/:id/recommendations',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const tenantId = parseInt(req.params.id);
  ...
  app.get(
    '/api/global-recommendations/category/:category',
    isAuthenticated,
    asyncHandler(async (req, res) => {
  ...
  app.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      const existingRecommendation = await storage
        .getGlobalRecommendations()
        .then((recs) => recs.find((r) => r.id === Number(id)));
        ...
        if (tenantWidgetRecs.length > 0) {
          ...
          if (categoryChanged) {
            ...
            for (const twRec of tenantWidgetRecs) {
              // Log the changes
              console.log(
                `Updating tenant widget recommendation ${twRec.id} from ${twRec.widgetType} to ${req.body.category.toUpperCase()}`
              );
          ...
          if (existingRecommendation.priority !== req.body.priority) {
            console.log(
              `Priority changed for recommendation ${id} from ${existingRecommendation.priority} to ${req.body.priority}`
            );
            ...
            console.log(
              `Priority updated for global recommendation ${id}. This will automatically reflect in widget displays.`
            );
  ...
  app.get(
    '/api/tenants/:id/secure-score-history',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const tenantId = parseInt(req.params.id);
  ...
  app.get(
    '/api/tenants/:id/secure-score-history/period',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const userId = (req.user as any).id;
      const tenantId = parseInt(req.params.id);
  ...
  app.get(
    '/api/tenants/:id/test-secure-score-history',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        try {
          for (let i = 0; i < 90; i++) {
            ...
            console.log(
              `Creating entry ${i + 1}/90: date=${date.toISOString()}, score=${score}, percent=${scorePercent}`
            );
  ...
  app.get(
    '/api/tenants/:id/current-secure-score',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      try {
        ...
        if (!connection) {
          ...
          return res.status(404).json({
            error: 'No Microsoft 365 connection found for this tenant',
          });
        }
        ...
        res.status(500).json({
          error: 'Failed to fetch secure score data',
  ...
  app.get(
    '/api/tenants/:id/secure-score-recommendations',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      try {
        ...
        res.status(500).json({
          error: 'Failed to fetch secure score recommendations',
          details: error.message,
        });
      }
    })
  ...
  app.get(
    '/api/tenants/:tenantId/secure-score-history/:year/:quarter',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const history = await storage.getSecureScoreHistoryForPeriod(Number(tenantId), startDate, endDate);
  ...
  app.get(
    '/api/tenants/:tenantId/secure-score-latest',
    isAuthenticated,
    asyncHandler(async (req, res) => {
  ...
  app.get(
    '/api/tenants/:tenantId/widget-recommendations',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      const { tenantId } = req.params;
  ...
  app.get(
    '/api/tenants/:tenantId/widget-recommendations/:widgetType',
    isAuthenticated,
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const widgetRecommendations = await storage.getTenantWidgetRecommendationsByWidgetType(
          Number(tenantId),
        ...
        res.status(500).json({
          message: 'Failed to fetch tenant widget recommendations by type',
        });
      }
    })
  );
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if (recommendationData.widgetType?.toUpperCase() === 'SECURE_SCORE') {
          return res.status(403).json({
            message:
              'Manual assignment of recommendations to the Secure Score widget is not allowed. Microsoft secure score recommendations are automatically displayed.',
          });
        }
  ...
  app.put(
    '/api/tenants/:tenantId/widget-recommendations/:id',
    isAuthenticated,
    isAuthorized([UserRoles.ADMIN, UserRoles.ANALYST]),
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        const updatedData = {
          ...
          ...(recommendationData.widgetType && {
            widgetType: recommendationData.widgetType.toUpperCase(),
          }),
        };

        const updatedRecommendation = await storage.updateTenantWidgetRecommendation(Number(id), updatedData);
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if (existingRecommendation.widgetType === 'SECURE_SCORE') {
          return res.status(403).json({
            message:
              'Manual removal of recommendations from the Secure Score widget is not allowed. Microsoft secure score recommendations are automatically managed.',
          });
        }
